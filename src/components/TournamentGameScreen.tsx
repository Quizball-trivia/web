...
import { TournamentLiveTracker } from './TournamentLiveTracker';
...
export function TournamentGameScreen({
  ...
  const handleAnswer = (answerIndex: number) => {
    ...
    if (isCorrect) {
      ...
      setPlayerScore((prev) => prev + Math.floor(speedBonus));
    }
    ...
    if (opponentAnswered) {
      ...
      if (opponentCorrect) {
        ...
        setOpponentScore((prev) => prev + Math.floor(opponentPoints));
      }
    }
  ...
  const handleNext = () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      onGameEnd(won, playerScore, opponentScore);
    }
  };
