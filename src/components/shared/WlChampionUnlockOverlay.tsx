...
const PLACE_TITLE_KEYS: Record<WlMedalPlace, MessageKey> = {
  1: "wlAward.title1",
  2: "wlAward.title2",
  3: "wlAward.title3",
};

const PLACE_ACCENTS: Record<WlMedalPlace, string> = {
  1: "#FFD700",
  2: "#D8D8D8",
  3: "#FF9600",
};
...
interface WlChampionUnlockOverlayProps {
  place: WlMedalPlace;
  open: boolean;
  /** e.g. "9 აგვისტო" — which weekend was won. */
  weekLabel?: string;
  onClose: () => void;
}

interface ConfettiPiece {
  x: number; y: number; rotate: number; scale: number; color: string; delay: number;
}

/** The Weekend League podium ceremony — same theatre as the World Cup unlock
 *  (rays, confetti, coin stamp-in, shockwave), WL-branded. */
export function WlChampionUnlockOverlay({ place, open, weekLabel, onClose }: WlChampionUnlockOverlayProps) {
  const { t } = useLocale();
  ...
  const confetti = useMemo<ConfettiPiece[]>(
    () =>
      Array.from({ length: 40 }, (_, i) => {
        const angle = (i / 40) * Math.PI * 2 + rand(i * 7) * 0.3;
        const dist = 130 + rand(i * 7 + 1) * 200;
        return {
          ...
          <div className="relative flex w-full max-w-sm flex-col items-center px-6 text-center">
            ...
            {confetti.map((piece, i) => (
              <motion.span
                ...
                animate={{
                  x: piece.x, y: piece.y, opacity: [0, 1, 1, 0],
                  scale: piece.scale, rotate: piece.rotate,
                }}
            ...
            <motion.div
              ...
              transition={{
                delay: 0.35, type: "spring", stiffness: 260, damping: 17,
                opacity: { delay: 0.35, duration: 0.22, ease: "easeOut" },
              }}
            >
              <WlChampionMedal place={place} className="w-52 drop-shadow-[0_10px_40px_rgba(0,0,0,0.6)]" />
              <motion.div
            ...
            >
              {weekLabel
                ? t("wlAward.unlockSubtitleWeek", { week: weekLabel })
                : t("wlAward.unlockSubtitle")}
            </motion.p>
