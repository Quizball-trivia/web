...
interface EventsDashboardProps {
  ...
  weekendLeagueHref?: string;
}

export function EventsDashboard({
  playerCoins = 2450,
  playerRankPoints = 1850,
  playerTier = 'Gold',
  tournaments = MOCK_EVENTS,
  ...
  const upcomingEvents = tournaments.filter(t => t.status === 'upcoming' && t.id !== featuredEvent.id);
  const pastEvents = tournaments.filter(t => t.status === 'ended' && t.id !== featuredEvent.id);
  ...
  const handleEnter = (event: Tournament) => {
    handleCloseSheet();
    if (event.id === featuredEvent.id) router.push(weekendLeagueHref);
  };
               ...
               <>
                  ...
                  <SheetFooter className="p-6 border-t border-border mt-auto bg-card/50">
                     <div className="w-full space-y-4">
                        {selectedEvent.status !== 'ended' && !isEligible(selectedEvent) && (
                        ...
                        <Button
                           ...
                           disabled={selectedEvent.status === 'ended' || !isEligible(selectedEvent)}
                           onClick={() => handleEnter(selectedEvent)}
                        >
                           {selectedEvent.status === 'ended' ? 'Ended' : isEligible(selectedEvent) ? (
                              selectedEvent.entryCoins === 0
