...
export function SquadSpin() {
  ...
  return (
    <MiniGameShell
      ...
      accent="#FFE500"
      headerRight={<StatPill label="Points" value={points.toLocaleString()} color="#FFE500" />}
    >
      {/* Reels */}
      <div className="mt-2 grid gap-1.5 sm:gap-2" style={{ gridTemplateColumns: `repeat(${reels}, minmax(0, 1fr))` }}>
        {activeReels.map((reel) => {
          ...
          const label = reel.charAt(0).toUpperCase() + reel.slice(1);
          return (
            <ReelCard
              key={reel}
              label={label}
        ...
        <AnimatePresence mode="wait">
          {phase === 'idle' && (
            <motion.div key="idle" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              ...
              <div className="flex gap-2">
                {DIFFICULTIES.map((d) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`font-poppins text-base font-black ${sel ? 'text-brand-yellow' : 'text-white'}`}>{d.reels} reels</span>
                      <span className="font-poppins text-[10px] font-bold uppercase tracking-wide text-white/45">
                        {d.label} · ×{difficultyMultiplier(d.reels)}
                      </span>
                    </button>
                  );
                })}
              ...
              >
                <RotateCw className="size-6" /> Spin
              </button>
            </motion.div>
          )}
          ...
          {phase === 'answering' && (
            <motion.div key="answer" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-3">
              ...
              <div className="flex items-center justify-between gap-2">
                <span className="font-poppins text-xs font-semibold text-white/40">Hold a reel, respin the rest</span>
                <button
                  type="button"
                  onClick={respin}
                ...
                >
                  <RotateCw className="size-3.5" /> Respin −{RESPIN_COST}
                </button>
              </div>
            </motion.div>
          )}
      ...
      {/* History strip */}
      <div className="mt-4">
        <div className="mb-1.5 font-poppins text-[10px] font-black uppercase tracking-wider text-white/35">Last 3 spins</div>
        <div className="flex gap-2">
          {[0, 1, 2].map((i) => {
            const h = history[i];
            return (
                ...
                {h ? (
                  <>
                    <span className="truncate font-poppins text-[10px] font-bold text-white/60">
                      {h.reels} reels · {h.combo.position}
                    </span>
                    {h.outcome === 'win' ? (
                      <>
                      ...
                      <span className="font-poppins text-[11px] font-black uppercase text-brand-red">Missed</span>
                    )}
                  </>
                ) : (
...
function ReelContent({ reel, display, iconSize }: { reel: keyof Display; display: Display; iconSize: number }) {
  ...
  if (reel === 'position')
    return (
      <>
        ...
        <ReelText>{POSITION_LABEL[display.position]}</ReelText>
      </>
    );
  ...
  return (
    <>
      ...
      <ReelText>{TROPHY_META[display.trophy].short}</ReelText>
    </>
  );
}
    ...
    >
      ...
      {holdable && onHold && (
        <button
          type="button"
          onClick={onHold}
          aria-label={held ? 'Release reel' : 'Hold reel'}
  ...
  return (
    ...
    >
      <div className="rounded-2xl border-2 p-4 text-center" style={{ borderColor: win ? '#38B60E66' : '#FB310166', background: win ? 'rgba(56,182,14,0.08)' : 'rgba(251,49,1,0.06)' }}>
        {win ? (
          <>
            <div className="font-poppins text-sm font-black uppercase tracking-wide text-brand-green">Correct!</div>
            ...
            <div className="mt-2 flex flex-wrap items-center justify-center gap-2 font-poppins text-xs font-bold text-white/60">
              <span>Only <span className="text-white">{result.pct}%</span> named this</span>
              <span className="flex items-center gap-1 rounded-full bg-brand-yellow/15 px-2 py-1 text-brand-yellow"><Sparkles className="size-3" /> ×{rMult} rarity</span>
              {dMult > 1 && <span className="rounded-full bg-brand-purple/20 px-2 py-1 text-brand-purple">×{dMult} {result.reels} reels</span>}
            </div>
            <div className="mt-2 font-poppins text-3xl font-black text-brand-green">+{result.points}</div>
          </>
        ) : (
          <>
            <div className="font-poppins text-sm font-black uppercase tracking-wide text-brand-red">Time&apos;s up</div>
            <div className="mt-1.5 font-poppins text-xs font-semibold text-white/55">
              {result.valid.length ? 'You could’ve said:' : 'No valid player for that combo!'}
            </div>
