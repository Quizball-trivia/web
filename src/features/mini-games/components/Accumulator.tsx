...
export function Accumulator({ backHref }: { backHref?: string } = {}) {
  ...
  const POOL = useMemo(() => getTrivia(miniLocale).slice(0, 8), [miniLocale]);
  const legs = useMemo(() => picked.map((id) => POOL.find((q) => q.id === id)!), [picked, POOL]);
  ...
  return (
    <MiniGameShell
      backHref={backHref}
      title={t('Accumulator')}
      subtitle={t('Five legs. One stake. All must land.')}
      accent="#58CC02"
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === 'select' && (
          <motion.div key="select" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-2 flex flex-1 flex-col">
            <div className="mb-2 flex items-center justify-between">
              <span className="font-poppins text-xs font-black uppercase tracking-wide text-white/50">{t('Pick 5 legs · {n}/5', { n: picked.length })}</span>
            ...
            <div className="mt-auto pt-4">
              ...
              >
                {t('Continue · {odds}x', { odds: formatOdds(combinedOdds) })}
              </button>
            </div>
          </motion.div>
        )}
        ...
        {phase === 'stake' && (
          <motion.div key="stake" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-4 space-y-4">
            <ReturnPanel stake={stake} secured={stake} potential={potentialReturn} label={t('Potential return')} leg={0} total={5} />
            <div>
              <div className="mb-2 font-poppins text-xs font-black uppercase tracking-wide text-white/50">{t('Your stake')}</div>
        ...
        {phase === 'play' && legs[leg] && (
          <motion.div key={`play-${leg}`} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} className="mt-4 space-y-4">
            <ReturnPanel stake={stake} secured={securedReturn} potential={potentialReturn} label={t('Return so far')} leg={leg} total={legs.length} />
        ...
        {phase === 'cashout' && (
          <motion.div key="cashout" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="mt-4 space-y-4">
            <ReturnPanel stake={stake} secured={securedReturn} potential={potentialReturn} label={t('4 of 5 landed!')} leg={leg} total={legs.length} />
            <div className="rounded-2xl border-2 border-brand-yellow/40 bg-brand-yellow/[0.06] p-4 text-center">
              <div className="font-poppins text-sm font-black uppercase text-brand-yellow">{t('Take the money or go for it?')}</div>
              <p className="mt-1 font-poppins text-xs font-semibold text-white/55">{t('One leg left. Cash out now, or risk it for the full {amount}.', { amount: money(potentialReturn) })}</p>
            </div>
        ...
        {phase === 'result' && result && (
          <motion.div key="result" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-6 flex flex-col items-center gap-4 text-center">
            ...
            <div className="font-poppins text-2xl font-black uppercase" style={{ color: result.won ? '#58CC02' : '#FB3101' }}>
              {result.won ? t('Bet won!') : t('Busted on leg {n}', { n: result.bustLeg ?? '' })}
            </div>
            {result.won ? (
              <div className="font-poppins text-4xl font-black tabular-nums text-brand-green">+{money(result.payout)}</div>
            ) : (
              <div className="font-poppins text-sm font-semibold text-white/50">{t('Lost your {stake} stake — one leg let it down.', { stake: money(stake) })}</div>
            )}
  ...
  return (
    <div className="rounded-2xl border-2 border-brand-green/30 bg-gradient-to-b from-brand-green/[0.1] to-transparent p-4 text-center">
      ...
      <div className="mt-1 flex items-center justify-center gap-1.5 font-poppins text-[11px] font-bold text-white/45">
        <TrendingUp className="size-3" /> {t('if it lands:')} <span className="text-white">{money(potential)}</span>
        <span className="text-white/25">·</span> {t('{leg}/{total} legs', { leg, total })}
      </div>
    </div>
  );
}
...
function QuestionBlock({
  ...
  return (
    <div className="rounded-2xl border border-white/[0.08] bg-surface-card/60 p-4">
      <div className="mb-2 flex items-center justify-between">
        <span className="font-poppins text-[10px] font-black uppercase tracking-wider" style={{ color: accent }}>{t('Leg {n}', { n: legNo })}</span>
