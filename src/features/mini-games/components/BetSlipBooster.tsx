...
import { SLIP, BOOST_FACTOR } from '../data/betSlip';
...
export function BetSlipBooster() {
  ...
  return (
    <MiniGameShell
      ...
      accent="#FFD700"
      headerRight={<StatPill label="Points" value={points.toLocaleString()} color="#FFD700" />}
    >
      {/* Slip */}
      <div className="mt-2 overflow-hidden rounded-2xl border-2 border-brand-gold/25 bg-surface-card/60">
        <div className="flex items-center justify-between border-b border-white/[0.06] bg-brand-gold/[0.06] px-4 py-2">
          <span className="font-poppins text-xs font-black uppercase tracking-wider text-brand-gold">Bet Slip · Treble</span>
          <span className="font-poppins text-[10px] font-bold uppercase text-white/40">3 selections</span>
        </div>
            ...
            return (
              <div key={s.club} className={`flex items-center gap-3 px-4 py-2.5 transition-colors ${active ? 'bg-brand-gold/[0.06]' : ''}`}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate font-poppins text-sm font-black text-white">{s.pick}</div>
                ...
                <div className="shrink-0 text-right">
                  ...
                  {isBoosted && <div className="font-poppins text-[9px] font-black uppercase text-brand-green">boosted</div>}
                </div>
              </div>
            );
          })}
        ...
        <div className="flex items-center justify-between border-t border-white/[0.06] bg-black/20 px-4 py-2.5">
          <span className="font-poppins text-xs font-black uppercase tracking-wider text-white/50">Total odds</span>
          ...
          {phase === 'boost' && (
            <motion.div key={`leg-${leg}`} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} className="rounded-2xl border border-white/[0.08] bg-surface-card/60 p-4">
              <div className="mb-2 flex items-center gap-1.5 font-poppins text-[10px] font-black uppercase tracking-wider text-brand-gold">
                <Zap className="size-3.5" /> Boost leg {leg + 1} · {SLIP[leg].club.replace(/ (CF|FC)$/i, '')}
              </div>
            ...
            <motion.div key="review" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-3">
              <div className="rounded-2xl border-2 border-brand-gold/40 bg-brand-gold/[0.08] p-4 text-center">
                <div className="flex items-center justify-center gap-1.5 font-poppins text-sm font-black uppercase text-brand-gold">
                  <TrendingUp className="size-4" /> Boosted {formatOdds(baseCombined)} → {formatOdds(boostedCombined)}
                </div>
                <div className="mt-1 font-poppins text-xs font-semibold text-white/55">
                  Stake {money(STAKE)} · potential return <span className="font-black text-brand-gold">{money(STAKE * boostedCombined)}</span>
                </div>
              </div>
          ...
          {phase === 'result' && result && (
            <motion.div key="result" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center gap-3 py-3 text-center">
              ...
              <div className="font-poppins text-2xl font-black uppercase" style={{ color: result.won ? '#58CC02' : '#FB3101' }}>
                {result.won ? 'Slip landed!' : 'Not this time'}
              </div>
              <div className="font-poppins text-sm font-semibold text-white/55">
                {result.won ? (
                  ...
                  `The boosted ${formatOdds(boostedCombined)}x didn’t come in.`
                )}
              </div>
