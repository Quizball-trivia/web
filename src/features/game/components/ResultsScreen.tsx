...
import { Trophy, Target, Flame, Star, TrendingUp, TrendingDown, Award } from 'lucide-react';
import { Achievement, MultiplayerMatch } from '@/types/game';
import { motion } from 'motion/react';

const STARTING_MONEY = 1_000_000;
const STARTING_MONEY_DISPLAY = `$${STARTING_MONEY / 1_000_000}M`;
...
export function ResultsScreen({
  ...
  const accuracy = isMoneyDropMode || totalQuestions === 0 ? 0 : Math.round((correctAnswers / totalQuestions) * 100);
  const getRankText = () => {
    if (isMoneyDropMode) {
      const percentage = (finalMoney / STARTING_MONEY) * 100;
  ...
  if (isMultiplayer && multiplayerMatch) {
    // Derive both playerWon and isDraw from the canonical winner field
    // Fall back to roundsWon comparison only if winner is absent/undefined
    const winner = multiplayerMatch.winner;
    ...
    if (winner !== undefined && winner !== null) {
      ...
      // Fallback: derive from roundsWon when winner is absent
      const p1Rounds = multiplayerMatch.player1.roundsWon;
      const p2Rounds = multiplayerMatch.player2.roundsWon;
      isDraw = p1Rounds === p2Rounds;
      playerWon = !isDraw && p1Rounds > p2Rounds;
    }
            ...
            <CardContent className="pt-6 pb-6">
              <div className="text-center mb-6">
                ...
                <p className="text-muted-foreground">
                  {multiplayerMatch.matchType === 'ranked' ? 'Ranked Match' : multiplayerMatch.matchType === 'friendly' ? 'Friendly Match' : 'Match'}
                </p>
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {multiplayerMatch.roundResults.map((result) => (
                  <div key={result.roundNumber} className="flex items-center justify-between p-3 bg-secondary rounded-lg">
                    <div className="flex items-center gap-2">
                      <span className="text-sm">Round {result.roundNumber}</span>
                      <Badge variant="outline" className="text-xs">
                        {MODE_LABELS[result.mode] || 'Categories'}
                      </Badge>
                    </div>
                    <div className="flex items-center gap-3">
            ...
            {isMoneyDropMode ? (
              <div>
                ...
                <div className="text-sm text-muted-foreground mt-1">
                  {((finalMoney / STARTING_MONEY) * 100).toFixed(1)}% retained
                </div>
              </div>
            ) : (
              <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Card>
            <CardContent className="pt-4 pb-4 text-center">
              ...
              <div className="text-xl">{STARTING_MONEY_DISPLAY}</div>
            </CardContent>
          </Card>
