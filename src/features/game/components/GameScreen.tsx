...
import { MinimalPlayerScores } from './MinimalPlayerScores';
...
export function GameScreen({ mode, questions, onGameEnd, onQuit, opponentName, opponentAvatar, isMultiplayer = false }: GameScreenProps) {
  ...
  const handleNextQuestion = () => {
    setGameState(prev => {
      ...
      // Game over when all questions answered (lives check handled by useEffect)
      if (nextIndex >= questions.length) {
        onGameEnd(prev.score, prev.correctAnswers, prev.streak);
        return prev;
      }
