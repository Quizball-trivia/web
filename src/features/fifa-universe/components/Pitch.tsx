...
export function Pitch({ squad, active, compact = false, tint = '#1645FF' }: { squad: Squad; active?: string | null; compact?: boolean; tint?: string }) {
  return (
    <div className={`relative mx-auto w-full overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-fut-pitch to-surface-mode-trough-deep ${compact ? 'aspect-[4/3] max-w-[320px]' : 'aspect-[4/3] max-w-sm lg:aspect-[3/4]'}`}>
            ...
            {card ? (
              <motion.div initial={{ scale: 0.6, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 320, damping: 20 }}>
                ...
                <div className={`flex flex-col items-center ${compact ? '' : 'lg:hidden'}`}>
                  <span className="flex size-9 items-center justify-center rounded-full border-2 border-fut-border/50 font-poppins text-sm font-black text-fut-ink-deep shadow-md" style={{ background: 'linear-gradient(160deg, #f7e7a8 0%, #d3ab46 100%)' }}>{card.overall}</span>
